...
func getBridgeInfo(br *netlink.Bridge, family int) (string, string, error) {
	addrs, err := netlink.AddrList(br, family)
	if err != nil && err != syscall.ENOENT {
		return "", "", fmt.Errorf("could not get list of IP addresses: %v", err)
	}

	println("Called getBridgeInfo ")
	...
	println("Return from getBridgeInfo ")
	
	return brIP, brDev, nil
}
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	if err != nil {
        ...
        /*
         * Get CNI_ARGS  environment variable supplied to the plugin
         */
	cni_args := os.Getenv("CNI_ARGS")
	if cni_args == "" {
	   return fmt.Errorf("plugin requires CNI_ARGS set")
	}
	...
	if derivedIP.String() != cniIP[1] {
	    println("derivedIP.String != cniIP[1]", derivedIP.String(), cniIP[1])
	    //return err
	}
	
	viaIP, Uplink, err := getBridgeInfo(br, netlink.FAMILY_V4)
	if err != nil {
	    println("failed to get bridge addr: ", err)
	}

	println(" add - viaIP", viaIP)
	println(" add - Uplink", Uplink)
